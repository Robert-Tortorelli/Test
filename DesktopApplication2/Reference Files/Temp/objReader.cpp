...
int objReader(void)
{
	...
	while (getline(obj, stringtext))						// Read an entire statement, from the input file stream object obj, into the string variable stringtext. At eof getline becomes false and the while loop is exited.
	{
		...
		{
			// The statement read is a face element statement, therefore all vertex attribute statements in the Wavefront .obj file have previously been read and parsed.
			...
			for (int i = 0; i <= 2; i++)
			{
				...
				//
				// Update index variable OurIndicesi of array variable OurIndices[OurIndicesi].
				OurIndicesi++;
				//
				// Update array variable OurIndices[OurIndicesi].
				...
				if (unique)
				{
					...
					// 
					// Update index variable OurVerticesi of array variable OurVertices[OurVerticesi].
					OurVerticesi++;
