...
int objReader(void)
{
	...
	while (getline(obj, stringtext))						// Read an entire statement, from the input file stream object obj, into the string variable stringtext. At eof getline becomes false and the while loop is exited.
	{
		...
		} else if (type == "f")
		{
			...
			//   Each of the three triplets (e.g., v1/vt1/vn1) in the face element statement consists of three positive numbers referring to three vertex attribute statements specifying one triangle vertex, which together comprise one set of vertex attributes. This set of vertex attributes is stored in the next sequential element of the array variable OurVertices, which is pointed to by array variable OurIndices.
			//   This results in storing vertex attributes in array variable OurVertices in the order that the face element statements appear in the Wavefront .obj file, which is the order in which the triangles must be drawn.
			...
			// Parse each of the three triplets in the face element.
			for (int i = 0; i <= 2; i++)
			{
				...
				// The face element indices are decremented by 1 because the C++ index variables v1, vti, and vni of the vertex attribute intermediate array variables v[vi], vt[vti], and vn[vn1] start with 0.
				//
				...
				//
				// Update the index variable OurIndicesi of array variable OurIndices[OurIndicesi].
				OurIndicesi++;
				OurIndices.emplace_back();

				// Assign the next element of the array variable OurIndices.
				int delta[3] = { 2, 2, -1 };				// The algorithm described above is implemented using the array variable delta.
				if (OurIndicesi != 0)
				{
					OurIndices[OurIndicesi] = OurIndices[OurIndicesi - 1] + delta[i]; // OurIndices[OurIndicesi] = OurIndices[OurIndicesi - 1] + delta[i]; starting with OurIndices[1] = OurIndices[1 - 1] + delta[1] == OurIndices[0] + 2 == 0 + 2 == 2 (that is, OurIndices[1] = 2).
				}
				else										// (OurIndicesi != 0)
				{
					OurIndices[OurIndicesi] = 0;			// OurIndices[0] = 0 is hardcoded instead of calculated.
				}
				// (Alternatively, the algorithm described above could be used to calculate the non-sequential index of the next sequential value of the array variable OurIndices, e.g., OurIndices[0] = 0 , OurIndices[2] = 1, OurIndices[1] = 2, OurIndices[3] = 3, OurIndices[5] = 4, OurIndices[4] = 5, etc. (This is not done in this program))
				...
				for (int j = 0; j <= OurVerticesi; j++)
				{
						...
						OurVertices[j].VertexNormalVector.z ==			  vn[fvn].z * -1.0f) // Invert the vertex normal vector's Z coordinate, to adjust it the Wavefront .obj file format to the DirectX format.
					{
						...
						unique = false;
						OurIndices[OurIndicesi] = j;		// Assign to array variable OurIndices[OurIndicesi] the index (the value of variable j) of the element of array variable OurVertices[j], which contains the set of vertex attributes previously found.
						break;
					}
				}
				...
				if (unique)
				{
					...
					// 
					// Update the index variable OurVerticesi of array variable OurVertices[OurVerticesi].
					OurVerticesi++;
					OurVertices.emplace_back();
